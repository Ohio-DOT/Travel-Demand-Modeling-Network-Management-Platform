import React, { useState } from "react";

export default function UploadChangeModal({ isOpen, onClose, onSubmit }) {
  const [file, setFile] = useState(null);
  const [isDisabled, setIsDisabled] = useState(false);

  const handleSubmit = () => {
    if (file) {
      setIsDisabled(true);
      onSubmit({ 
        "file":file
      });
      
      document.getElementById("getChangeFile").value = "";
      setFile(null);
      // onClose();
    } else {
      setIsDisabled(false);
      alert("Please upload a file and enter all information requested.");
    }
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-xl shadow-xl">
        <h2 className="text-xl font-semibold mb-4">Upload Changesets (.json)</h2>
        <p className="text-sm mb-4">Only one .json file accepted at a time.</p>

        <div className="mb-4">
          <label className="block text-sm font-medium mb-1">Select File:</label>
          <input
            type="file"
            id="getChangeFile"
            // multiple
            accept=".json"
            onChange={(e) => setFile(e.target.files[0])}
            required
            className="block w-full border rounded p-2"
          />
        </div>

        <div className="flex justify-end space-x-2">
          <button
            onClick={onClose}
            // disabled={isDisabled}
            className="px-4 py-2 rounded bg-gray-200 hover:bg-gray-300 text-sm/6 font-semibold shadow-xs"
          >
            Cancel
          </button>
          <button
            onClick={handleSubmit}
            // disabled={isDisabled}
            className="px-4 py-2 rounded bg-emerald-600 hover:bg-emerald-700 text-sm/6 font-semibold text-white shadow-xs"
          >
            Submit
          </button>
        </div>
      </div>
    </div>
  );
}
